...
(function () {
  ...
  var COURSES = [
    ...
    { name: 'Kurmancî A2', href: '#', status: 'locked' },
  ...
  var UNITS = [
    ...
    { name: 'Beş 3: Reng û Danasîn', href: '#', status: 'locked' },
  ...
  var LESSONS = [
    ...
    { name: 'Ders 3: Ez... im', href: '#', status: 'locked' },
  ...
  var PRACTICE_SVG = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>';
  ...
  function buildDropdown(id, items, currentName) {
    ...
    items.forEach(function (item) {
      ...
      if (item.status === 'locked') {
        ...
        if (isActive && item.status !== 'done') html += '';
        html += '<span>' + item.name + '</span>';
        html += '</a>';
      }
    });
  ...
  function buildBreadcrumb(el) {
    ...
    var closeHref = el.getAttribute('data-close-href') || '';
    ...
    // Right: action icons
    html += '<div class="breadcrumb-actions">';
    html += '<button class="breadcrumb-action-btn" title="Quick Practice" style="opacity:0.4;cursor:default;">';
    html += PRACTICE_SVG;
    html += '</button>';
    if (closeHref) {
      html += '<a href="' + closeHref + '" class="breadcrumb-action-btn close-btn" title="Close">';
      html += CLOSE_SVG;
      html += '</a>';
    }
    html += '</div>';

# Multi-Language Architecture — HinbunaKurdi
...
For `courses`, `modules`, `units`, `lessons` — entities with flat, predictable text fields.

```sql
-- Base entity: non-translatable fields only
CREATE TABLE lessons (
  id UUID PRIMARY KEY,
  unit_id UUID REFERENCES units(id),
  slug VARCHAR(200),
...
-- Translation table: all translatable text
CREATE TABLE lesson_translations (
  id UUID PRIMARY KEY,
  lesson_id UUID NOT NULL REFERENCES lessons(id) ON DELETE CASCADE,
  locale VARCHAR(10) NOT NULL,
...
For `activities.content` and `materials` — deeply nested, variable structure per activity type.

```sql
CREATE TABLE activities (
  id UUID PRIMARY KEY,
  lesson_id UUID REFERENCES lessons(id),
  type VARCHAR(50),  -- 'mcq', 'gap_fill', 'matching', etc.
  order_index INT,
  content JSONB NOT NULL,
  /*
    Example (MCQ):
    {
      "question": {"ku": "Navê te çi ye?", "de": "Wie heißt du?"},
      "options": [
        {"ku": "Ez baş im", "de": "Mir geht es gut"},
        {"ku": "Navê min Azad e", "de": "Ich heiße Azad"}
      ],
      "correct_index": 1,
      "hints": [{"ku": "Bifikire...", "de": "Denk nach..."}]
    }
  */
  metadata JSONB,
  is_published BOOLEAN,
...
New fields in `app_settings` for deployment-level language control:

```sql
-- Add to app_settings or create dedicated table
CREATE TABLE supported_locales (
  id UUID PRIMARY KEY,
  locale VARCHAR(10) NOT NULL UNIQUE,   -- 'ku', 'de', 'en', 'sv'
  display_name VARCHAR(100) NOT NULL,   -- 'Kurdî (Kurmancî)', 'Deutsch'
  direction VARCHAR(3) DEFAULT 'ltr',   -- 'ltr' or 'rtl'
  color VARCHAR(7),                     -- '#2D5A3D' (for UI chips)
...
ALTER TABLE app_settings ADD COLUMN translations_enabled BOOLEAN DEFAULT TRUE;
ALTER TABLE app_settings ADD COLUMN source_locale VARCHAR(10) DEFAULT 'ku';
ALTER TABLE app_settings ADD COLUMN default_locale VARCHAR(10) DEFAULT 'de';
```

### Tier 4: User Language Preference

```sql
-- Add to users table or user_learning_preferences
ALTER TABLE users ADD COLUMN ui_locale VARCHAR(10) DEFAULT 'de';          -- for i18n (UI labels)
ALTER TABLE users ADD COLUMN content_locale VARCHAR(10) DEFAULT 'de';     -- for lesson translations
ALTER TABLE users ADD COLUMN translations_visible BOOLEAN DEFAULT TRUE;   -- can disable (immersion mode)
```
...
## TypeORM Integration (NestJS)

The `typeorm-translatable` library supports Pattern C natively:
...
For activities, TypeORM's `@Column({ type: 'jsonb' })` handles JSONB natively.
...
Changes needed to `_docs/04-data-model-design.md`:

| Current | Change |
|---------|--------|
| `courses.name` (string) | Move to `course_translations.name` |
| `courses.description` (text) | Move to `course_translations.description` |
| `modules.name`, `.description` | Move to `module_translations` |
| `units.name`, `.description` | Move to `unit_translations` |
| `lessons.name`, `.description`, `.learning_objective` | Move to `lesson_translations` |
| `activities.content` (JSON) | Keep — JSONB with `{ locale: value }` keys |
| `materials.metadata` (JSON) | Keep — JSONB with `{ locale: value }` keys |
| `app_settings` | Add `translations_enabled`, `source_locale`, `default_locale` |
| (new) `supported_locales` table | Language registry for the deployment |
| `users` | Add `ui_locale`, `content_locale`, `translations_visible` |

**New tables:** 5 translation tables + 1 locale registry = 6 new tables
**Modified tables:** `app_settings`, `users` (or `user_learning_preferences`)
